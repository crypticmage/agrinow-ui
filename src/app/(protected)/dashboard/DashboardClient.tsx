...
import { dashboardStatsConfig } from "@/data/dashboard";

const DashboardContent = () => {
  const role = useAppStore((s) => s.currentUser?.role);
  // Only show loading if we really need to (e.g. first mount on client if not SSR'd)
  const simulatedLoading = useSimulatedLoading(800);
  const loading = simulatedLoading; 
      ...
      <div className="space-y-6 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          {loading
            ...
            : dashboardStatsConfig.map((stat, i) => {
                let displayValue = (stat as any).value;
                if ((stat as any).key === "staffActive") {
                  displayValue = `${mockStaff.filter((s) => s.status === "active").length}`;
                } else if ((stat as any).key === "weather") {
                  displayValue = `${mockWeather.temp}°C`;
                }
                ...
                return (
                  ...
                  >
                    <Card className="group relative overflow-hidden transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 border border-border shadow-sm bg-card">
                      <div
